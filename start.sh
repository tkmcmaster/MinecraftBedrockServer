...
Permissions=$(sudo bash /opt/minecraft/fixpermissions.sh -a)
...
if [ "$?" != 0 ]; then
    ...
    if [[ "$PinFile" == *"zip" ]] && [[ "$InstalledFile" == "$PinFile" ]]; then
        ...
        if [ ! -z "$DownloadFile" ]; then
            if [ ! -e /opt/minecraft/server.properties ]; then
            ...
            Permissions=$(chmod u+x /opt/minecraft/bedrock_server >/dev/null)
...
if [ ! -e /opt/minecraft/allowlist.json ]; then
    echo "Creating default allowlist.json..."
    echo '[]' >/opt/minecraft/allowlist.json
fi
if [ ! -e /opt/minecraft/permissions.json ]; then
    echo "Creating default permissions.json..."
    echo '[]' >/opt/minecraft/permissions.json
fi
ContentLogging=$(grep "content-log-file-enabled" /opt/minecraft/server.properties)
if [ -z "$ContentLogging" ]; then
    echo "" >> /opt/minecraft/server.properties
    echo "content-log-file-enabled=true" >> /opt/minecraft/server.properties
    echo "# Enables logging content errors to a file" >> /opt/minecraft/server.properties
fi
...
if [[ "$CPUArch" == *"aarch64"* ]]; then
    ...
    if [ -n "$(which box64)" ]; then
        ...
        BASH_CMD="LD_LIBRARY_PATH=/opt/minecraft /opt/minecraft/bedrock_server"
    fi
else
    BASH_CMD="LD_LIBRARY_PATH=/opt/minecraft /opt/minecraft/bedrock_server"
fi
